...
{#if showOffers}
  <div class="fixed inset-0 bg-black/60 z-50 flex items-center justify-center ">
    <div class="bg-white dark:bg-gray-900 text-gray-800 dark:text-white w-full max-w-5xl mx-4 p-8 rounded-lg shadow-lg relative animate-fade-in-up overflow-y-auto max-h-[90vh]">
      <!-- Close Button -->
      <button class="absolute top-4 right-4 text-gray-600 hover:text-red-600" on:click={() => (showOffers = false)}>
        <X class="w-6 h-6" />
      </button>
